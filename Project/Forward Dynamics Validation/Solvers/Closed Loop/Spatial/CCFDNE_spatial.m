function qdd = CCFDNE_spatial(model)
NB    = model.NB;
q     = model.q;
qd    = model.qd;
n0    = model.n0;
...
D = zeros(13*NB,7);
D(13*(NB-1)+1:13*(NB-1)+3,1:3) = eye(3);
D(13*(NB-1)+1:13*(NB-1)+3,4:6) = S(r(:,NB));
D(13*(NB-1)+4:13*(NB-1)+6,4:6) = eye(3);

E = zeros(7,13*NB);
E(1:3,13*(NB-1)+10:13*(NB-1)+12) = eye(3);
E(4:6,13*(NB-1)+7:13*(NB-1)+9) = eye(3);
