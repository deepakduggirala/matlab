function [B,qdd] = closedLoopReverse(model)
NB    = model.NB;
q     = model.q;
qd    = model.qd;
n0    = model.n0;
...
for k=1:NB
    ...
    A(10:12,10:12) = eye(3);
    A(13,1:3) = n(:,i)';
    B((k-1)*13+4:k*13+3,(k-1)*13+4:k*13+3) = A(:,:);
    if k<NB
        A = zeros(13,13);
        A(1:3,1:3) = -eye(3);
        A(1:3,4:6) = -S(r(:,i));
        A(4:6,4:6) = -eye(3);
        B((k)*13+4:(k+1)*13+3,(k-1)*13+4:k*13+3) = A(:,:);
        A = zeros(13,13);
        A(4:6,10:12) = m(i+1)*eye(3)/2;
        A(7:9,7:9) = -eye(3);
        A(10:12,10:12) = -eye(3);
        B((k-1)*13+4:k*13+3,(k)*13+4:(k+1)*13+3) = A(:,:);
    end
end
