...
C(13*NB+1:13*NB+3,1) = m(NB)*grav + m(NB)*S(w(:,NB))*S(w(:,NB))*r(:,NB);
...
% A = zeros(17,17);
% B(13*(NB-1)+1:13*NB+4,13*(NB-1)+1:13*NB+4) = A(:,:);
% A(1:3,1:3) = -eye(3);
% A(1:3,4:6) = S(r(:,NB));
% A(1:3,7:9) = Icm{NB};
% A(1:3,11:13) = eye(3);
...
B(13*(NB-1)+1:13*(NB-1)+3,13*NB+1:13*NB+3) = S(r(:,NB));
B(13*(NB-1)+4:13*(NB-1)+6,13*NB+1:13*NB+3) = eye(3);

B(13*NB+1:13*NB+3,13*(NB-1)+4:13*(NB-1)+6) = -eye(3);
B(13*NB+1:13*NB+3,13*(NB-1)+7:13*(NB-1)+9) = m(NB)*S(r(:,NB));
B(13*NB+1:13*NB+3,13*NB+1:13*NB+3) = eye(3);
x = B\C;
qdd = zeros(NB,1);
for i=1:NB
    qdd(i) = x(13*i);
