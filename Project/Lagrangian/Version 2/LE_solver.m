function [qdd,D,H,C] = LE_solver(model)
...
for i=1:nL
    sum = 0;
    for j=i:nL
      sum = sum + m(j)*[0 -9.80665 0 0]*U(:,:,j,i)*r0(:,j);
    end
    C(i) = sum;
end
end
function X = getH2(nL,thtd,J,U,R,RD,n0,tht)
X = zeros(nL,1);
for i=1:nL
    sum=0;
    for j = i:nL
        for k = 1:j
            for m=1:j
                sum = sum + trace(getU2(j,k,m,R,RD,n0,tht)*J(:,:,j)*(U(:,:,j,i)'))*thtd(k)*thtd(m);
            end
        end
    end
    X(i) = sum;
